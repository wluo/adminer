...
if (isset($_GET["vitess"])) {
	...
	define("DRIVER", "vitess");
	$vitessClassPath = __DIR__ . DIRECTORY_SEPARATOR . 'vitess' . DIRECTORY_SEPARATOR . 'Min' . DIRECTORY_SEPARATOR;

	if (class_exists('VitessPdo\PDO')) {
		class Min_DB extends \Adminer\Vitess\DB
		{}
	}

	class Min_Driver extends \Adminer\Vitess\Driver
	{}
	...
	function connect() {
		...
		$keyspace = $adminer->database();
		$server = explode($credentials[0], ":");
